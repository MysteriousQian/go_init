...
func Setup() {
	...
	if mode == "dev" {
		stripeKey = viper.GetString("server.devKey")
		endpointSecret = viper.GetString("server.devSecret")
		if stripeKey == "" || endpointSecret == "" {
			log.Fatalln("⚠️  server.devKey or server.devSecret is not set correctly in config file")
		}
	} else if mode == "prod" {
		stripeKey = viper.GetString("server.prodKey")
		endpointSecret = viper.GetString("server.prodSecret")
		if stripeKey == "" || endpointSecret == "" {
			log.Fatalln("⚠️  server.prodKey or server.prodSecret is not set correctly in config file")
		}
	} else {
